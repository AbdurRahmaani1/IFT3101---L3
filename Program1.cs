...
class MarkAndCompact
{
    ...
    static void Main()
    {
        ...
        for (int i = 0; i < Objets.Count; i++)
        {
            if (Objets[i].marque)
            {
                Objets[i].adresse = free;
                free += Objets[i].taille;
            }
            else
            {
        ...
        {
            System.Console.WriteLine(Environment.NewLine);
            System.Console.WriteLine(Objets[i].name + " (Adresse apres GC: " + Objets[i].adresse.ToString() + ")");
            if (Objets[i].Ref.Count > 0)
            {
                System.Console.WriteLine("  References: ");
                for (int j = 0; j < Objets[i].Ref.Count; j++)
